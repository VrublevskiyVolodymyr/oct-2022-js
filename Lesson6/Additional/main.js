...
let delete_characters=(str, length)=>{return str.substring(0,length)}
...
// -знайти наібльшу книжку.
  let booksTitle =  books.sort((book1,book2)=>{return book2.title.length-book1.title.length})
...
let booksMaxGenre= books.sort((book1,book2)=>{return book2.genre.length-book1.genre.length})
...
// - знайти книжку/ки які писали 2 автори
    let authors=books.filter((value)=>{return value.authors.length===2})
for (let i=0; i<authors.length; i++) {
    if(authors[i].title.length===authors[0].title.length)
console.log(authors[i].title)}
// - знайти книжку/ки які писав 1 автор
    let author=books.filter((value)=>{return value.authors.length===1})
        for(let i=0; i<author.length; i++){
                console.log(author[i].title)
}
// - вісортувати книжки по кількості сторінок по зростанню
 let booksSort= books.sort((book1,book2)=>{return book2.pageCount-book1.pageCount})
